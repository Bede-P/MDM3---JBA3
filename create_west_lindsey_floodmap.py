...
def get_west_lindsey_boundary():
    """
    Get West Lindsey administrative boundary from OpenStreetMap
    
    Returns:
    - GeoDataFrame with West Lindsey boundary in EPSG:27700
    """
    print("\n" + "="*60)
    print("FETCHING WEST LINDSEY ADMINISTRATIVE BOUNDARY")
    print("="*60)
    ...
    try:
        ...
        # Get West Lindsey boundary from OSM
        west_lindsey = ox.geocode_to_gdf("West Lindsey, England, UK")
        
        # Convert to EPSG:27700
        west_lindsey_27700 = west_lindsey.to_crs(epsg=27700)
        
        print(f"[SUCCESS] Boundary fetched via OSMnx")
        print(f"  Geometry type: {west_lindsey_27700.geometry.iloc[0].geom_type}")
        print(f"  Bounds: {west_lindsey_27700.total_bounds}")
        
        return west_lindsey_27700
    ...
    try:
        ...
        # Overpass query for West Lindsey administrative boundary
        overpass_query = """
        [out:json][timeout:25];
        (
          relation["name"="West Lindsey"]["admin_level"="8"]["boundary"="administrative"];
        );
        out geom;
        """
            ...
            if data.get('elements'):
                ...
                if 'members' in element:
                    ...
                    if coords:
                        ...
                        gdf = gpd.GeoDataFrame([{'name': 'West Lindsey'}], 
    ...
    # Create rectangular boundary for West Lindsey (approximate bounds)
    min_lon, max_lon = -0.8, -0.2
    min_lat, max_lat = 53.0, 53.5
    ...
    gdf = gpd.GeoDataFrame([{'name': 'West Lindsey (approx)'}], 
                           geometry=[bbox_geom], 
                           crs='EPSG:27700')
...
def load_roads_with_simple_flood_check(
    ...
    west_lindsey_gdf: gpd.GeoDataFrame,
    west_lindsey_bbox,
    west_lindsey_boundary,
    start_year: int,
    end_year: int
):
    """
    Load roads in West Lindsey area and check simple flood coverage (flooded vs not flooded)
    
    Parameters:
    - road_geojson_dir: Directory containing road GeoJSON files
    - west_lindsey_gdf: GeoDataFrame with flood data for West Lindsey (in EPSG:27700)
    - west_lindsey_bbox: Bounding box for initial filtering (performance)
    - west_lindsey_boundary: True administrative boundary geometry for precise filtering
    ...
    for idx, row in west_lindsey_gdf.iterrows():
        geom = row.geometry
    ...
    print(f"  Total flood outlines: {len(west_lindsey_gdf):,}")
    ...
    bbox_bounds = west_lindsey_bbox.bounds  # (minx, miny, maxx, maxy)
    ...
    for file_idx, road_file in enumerate(road_files, 1):
        try:
            ...
            roads_in_bbox = road_gdf[road_gdf.geometry.intersects(west_lindsey_bbox)]
            
            if len(roads_in_bbox) == 0:
                continue
            
            # Step 2: Precise filter using true administrative boundary
            roads_in_boundary = roads_in_bbox[roads_in_bbox.geometry.intersects(west_lindsey_boundary)]
...
def create_interactive_yearly_map(
    ...
    output_file: str = "West_Lindsey_Interactive.html"
):
    ...
    # Get West Lindsey boundary
    west_lindsey_boundary_gdf = get_west_lindsey_boundary()
    if west_lindsey_boundary_gdf is None:
        print("[ERROR] Could not fetch West Lindsey boundary")
        return
    
    west_lindsey_boundary = west_lindsey_boundary_gdf.geometry.iloc[0]
    bounds_27700 = west_lindsey_boundary_gdf.total_bounds
    min_x, min_y, max_x, max_y = bounds_27700
    west_lindsey_bbox = box(min_x, min_y, max_x, max_y)
    ...
    west_lindsey_gdf = gpd.clip(filtered_gdf, west_lindsey_boundary_gdf)
    
    years_with_floods = sorted([int(y) for y in west_lindsey_gdf["start_year"].unique()])
    
    print(f"\nFound {len(years_with_floods)} years with floods:")
    for year in years_with_floods:
        count = len(west_lindsey_gdf[west_lindsey_gdf["start_year"] == year])
        print(f"  {year}: {count} flood outlines")
    
    # Transform to WGS84
    west_lindsey_wgs84 = west_lindsey_gdf.to_crs(epsg=4326)
    west_lindsey_boundary_wgs84 = west_lindsey_boundary_gdf.to_crs(epsg=4326)
    
    boundary_wgs84_geom = west_lindsey_boundary_wgs84.geometry.iloc[0]
    ...
    # Add boundary
    boundary_geojson = json.loads(west_lindsey_boundary_wgs84.to_json())
    folium.GeoJson(
        boundary_geojson,
        ...
        name='West Lindsey Boundary'
    ).add_to(m)
    
    # Load and add Voronoi cells
    voronoi_file = "west_lindsey_school_map.geojson"
    ...
    if os.path.exists(voronoi_file):
        ...
        try:
            ...
            # Clip Voronoi cells to West Lindsey boundary
            voronoi_clipped = gpd.clip(voronoi_gdf, west_lindsey_boundary_wgs84)
    ...
    # Load schools and match to Voronoi cells
    schools_file = "west_lindsey_schools_list.csv"
    school_to_cell = {}
    if os.path.exists(schools_file) and voronoi_loaded and voronoi_clipped is not None:
        print("\nMatching schools to Voronoi cells...")
        try:
            import pandas as pd
            ...
            for idx, school_row in schools_df.iterrows():
                ...
                lon = school_row['Longitude']
                school_name = school_row['SCHNAME']
                ...
                school_point_wgs84 = Point(lon, lat)
                school_point_27700 = gpd.GeoSeries([school_point_wgs84], crs='EPSG:4326').to_crs(epsg=27700).iloc[0]
                
                # Find which cell contains this school
                for cell_idx, cell_row in voronoi_27700.iterrows():
                    if cell_row.geometry.contains(school_point_27700):
                        cell_id = cell_row['cell_id']
                        school_to_cell[cell_id] = school_name
                        break
            
            print(f"  Matched {len(school_to_cell)} schools to Voronoi cells")
        except Exception as e:
            print(f"  [WARNING] Could not match schools: {e}")
    
    # Store flood layers to add after Voronoi cells (so they appear on top)
    flood_layers_data = []
    flood_stats = {}
    for year in years_with_floods:
        year_floods = west_lindsey_wgs84[west_lindsey_wgs84["start_year"] == year]
    ...
    if include_roads and road_geojson_dir:
        ...
        flood_unions = {}
        for year in years_with_floods:
            year_floods = west_lindsey_gdf[west_lindsey_gdf["start_year"] == year]
            geoms = []
        ...
        for file_idx, road_file in enumerate(road_files, 1):
            ...
            try:
                ...
                roads_in_boundary = road_gdf[road_gdf.geometry.intersects(west_lindsey_boundary)]
        ...
        if voronoi_loaded and voronoi_clipped is not None:
            ...
            # Calculate flooded roads ratio for each cell by year
            # Reference: Calderdale uses road_line.intersects(flood_unions[year]) to check if road is flooded
            print("\nCalculating flooded roads ratio for each Voronoi cell by year...")
            for year in years_with_floods:
                if year not in flood_unions:
                    continue
                
                year_flood_union = flood_unions[year]
                ...
                for cell_idx, cell_row in voronoi_27700.iterrows():
                    ...
                    for road in roads_data:
                        ...
                        if road_geom.intersects(cell_geom):
                            # Calculate the length of road within this cell
                            try:
                                road_in_cell = road_geom.intersection(cell_geom)
                                
                                # Skip if intersection is empty or invalid
                                if road_in_cell.is_empty or not road_in_cell.is_valid:
                                    continue
                                
                                # Calculate length of road segment within cell
                                length_m = 0.0
                                if road_in_cell.geom_type == 'LineString':
                                    length_m = road_in_cell.length
                                elif road_in_cell.geom_type == 'MultiLineString':
                                    length_m = sum(line.length for line in road_in_cell.geoms)
                                else:
                                ...
                                if length_m > 0:
                                    ...
                                    # Check if the road segment within this cell intersects with flood
                                    # Reference: Calderdale uses road_line.intersects(flood_unions[year])
                                    if road_in_cell.intersects(year_flood_union):
                                        # Calculate the length of flooded portion within cell
                                        try:
                                            flooded_intersection = road_in_cell.intersection(year_flood_union)
                                            
                                            # Skip if intersection is empty or invalid
                                            if flooded_intersection.is_empty or not flooded_intersection.is_valid:
                                                # If intersection is empty but intersects() returned True,
                                                # it might be a precision issue. Use the full road_in_cell length.
                                                flooded_length_m += length_m
                                                continue
                                            
                                            flooded_segment_length = 0.0
                                            
                                            # Handle different geometry types from intersection
                                            if flooded_intersection.geom_type == 'LineString':
                                                flooded_segment_length = flooded_intersection.length
                                            elif flooded_intersection.geom_type == 'MultiLineString':
                                                flooded_segment_length = sum(line.length for line in flooded_intersection.geoms)
                                            elif flooded_intersection.geom_type == 'Polygon':
                                                # If intersection is a polygon, use its boundary length
                                                flooded_segment_length = flooded_intersection.boundary.length
                                            elif flooded_intersection.geom_type == 'MultiPolygon':
                                                flooded_segment_length = sum(poly.boundary.length for poly in flooded_intersection.geoms)
                                            elif flooded_intersection.geom_type == 'GeometryCollection':
                                                # Handle GeometryCollection (may contain LineString, Point, etc.)
                                                for geom in flooded_intersection.geoms:
                                                    if geom.geom_type == 'LineString':
                                                        flooded_segment_length += geom.length
                                                    elif geom.geom_type == 'MultiLineString':
                                                        flooded_segment_length += sum(line.length for line in geom.geoms)
                                                    elif geom.geom_type == 'Polygon':
                                                        flooded_segment_length += geom.boundary.length
                                                    elif geom.geom_type == 'MultiPolygon':
                                                        flooded_segment_length += sum(poly.boundary.length for poly in geom.geoms)
                                                    # For Point/MultiPoint, consider as small segments
                                                    elif geom.geom_type == 'Point':
                                                        flooded_segment_length += 5.0  # 5 meters per point
                                                    elif geom.geom_type == 'MultiPoint':
                                                        flooded_segment_length += len(geom.geoms) * 5.0
                                            elif flooded_intersection.geom_type == 'Point':
                                                flooded_segment_length = 5.0  # 5 meters per point
                                            elif flooded_intersection.geom_type == 'MultiPoint':
                                                flooded_segment_length = len(flooded_intersection.geoms) * 5.0
                                            
                                            # Only add if we got a meaningful length
                                            if flooded_segment_length > 0:
                                                flooded_length_m += flooded_segment_length
                                            else:
                                                # If intersection exists but has no length (e.g., only points),
                                                # use a small fraction of the road length
                                                flooded_length_m += length_m * 0.01  # 1% of road length
                                        except Exception as e:
                                            # If intersection calculation fails, but road is marked as flooded,
                                            # use the full length of road_in_cell as a conservative estimate
                                            if year in road.get('flood_years', []):
                                                flooded_length_m += length_m
                            except Exception as e:
                                # If intersection fails, try alternative method
                                # Check if road is marked as flooded for this year
                                if year in road.get('flood_years', []):
                                    try:
                                        # Try to get intersection length as fallback
                                        road_in_cell = road_geom.intersection(cell_geom)
                                        if not road_in_cell.is_empty and road_in_cell.is_valid:
                                            if road_in_cell.geom_type == 'LineString':
                                                length_m = road_in_cell.length
                                                total_length_m += length_m
                                                flooded_length_m += length_m
                    ...
                    # Calculate flooded roads ratio
                    if total_length_m > 0:
                        ratio = (flooded_length_m / total_length_m) * 100.0
                        voronoi_27700.at[cell_idx, f'flooded_roads_ratio_{year}'] = round(ratio, 2)
                    else:
                        voronoi_27700.at[cell_idx, f'flooded_roads_ratio_{year}'] = 0.0
    ...
    # Initialize cell flood ratios dictionary (used for JavaScript)
    cell_flood_ratios = {}
    ...
    if voronoi_loaded and voronoi_clipped is not None:
        ...
        for idx, row in voronoi_clipped.iterrows():
            cell_id = row['cell_id']
            total_km = row.get('road_length_km', 0.0)
            school_name = school_to_cell.get(cell_id, '')
            
            # Collect flooded roads ratio for each year
            cell_ratios = {}
            for year in years_with_floods:
                ratio_field = f'flooded_roads_ratio_{year}'
                if ratio_field in row:
                    cell_ratios[year] = float(row[ratio_field])
                else:
                    cell_ratios[year] = 0.0
            # Use string key to ensure JSON serialization works correctly
            cell_flood_ratios[str(cell_id)] = cell_ratios
            
            # Get initial ratio for first year with floods
            initial_year = years_with_floods[0] if years_with_floods else 2000
            initial_ratio = cell_ratios.get(initial_year, 0.0)
            ...
            if school_name:
                ...
                <div style="margin-bottom: 8px;">
                    <b style="color: #555;">Primary School:</b> 
                    <span style="color: #000; font-size: 13px; font-weight: bold;">{school_name}</span>
                </div>
                '''
            ...
            popup_html += f'''
                <div style="margin-bottom: 8px;">
                    ...
                    <span style="color: #000; font-size: 13px; font-weight: bold;">{total_km:.2f} km</span>
                </div>
                <div style="margin-bottom: 8px;">
                    <b style="color: #555;">Flooded Roads Ratio:</b> 
                    <span style="color: #FF0000; font-size: 13px; font-weight: bold;" id="floodRatio_{cell_id}">{initial_ratio:.2f} %</span>
                </div>
            ...
            # Also check for Unknown roads
            unknown_pct = row.get('Unknown_pct', 0.0)
            unknown_km = row.get('Unknown_km', 0.0)
    ...
    if os.path.exists(schools_file):
        ...
        try:
            ...
            # All schools in West Lindsey are Primary schools
            # Define school color (Royal Blue for Primary)
            school_color = '#4169E1'
            ...
            for idx, row in schools_df.iterrows():
                ...
                <div style="font-family: Arial; font-size: 12px; width: 220px;">
                    ...
                    <table style="width: 100%; font-size: 11px;">
                        <tr><td><b>Type:</b></td><td>Primary</td></tr>
                        <tr><td><b>Address:</b></td><td>{street}</td></tr>
                    </table>
                </div>
                """
    ...
    var cellFloodRatios = {json.dumps(cell_flood_ratios)};
    ...
    function switchYear() {{
        ...
        // Update flooded roads ratio in all visible popups
        for (var cellId in cellFloodRatios) {{
            var ratioElement = document.getElementById('floodRatio_' + cellId);
            if (ratioElement && cellFloodRatios[cellId] && cellFloodRatios[cellId][year] !== undefined) {{
                var ratio = cellFloodRatios[cellId][year];
    ...
    window.addEventListener('load', function() {{
        setTimeout(function() {{
            ...
            if (roadStats[{years_with_floods[0] if years_with_floods else 2000}]) {{
                var flooded = roadStats[{years_with_floods[0] if years_with_floods else 2000}].flooded;
                var notFlooded = roadStats[{years_with_floods[0] if years_with_floods else 2000}].not_flooded;
                var total = flooded + notFlooded;
                var percentage = total > 0 ? ((flooded / total) * 100).toFixed(2) : 0;
                document.getElementById('floodImpact').textContent = percentage + '%';
            }}
...
def create_west_lindsey_flood_map(
    ...
    """
    Generate flood outline map for West Lindsey area in given year range
    ...
    if output_file is None:
        if specific_year:
            output_file = f"West_Lindsey_{specific_year}.html"
        else:
            output_file = f"west_lindsey_flood_map_{start_year}_{end_year}.html"
    ...
    print("=" * 60)
    print("West Lindsey Flood Map (2000-2025)")
    print("=" * 60)

    # ----- 1. Get West Lindsey TRUE administrative boundary -----
    west_lindsey_boundary_gdf = get_west_lindsey_boundary()
    
    if west_lindsey_boundary_gdf is None or len(west_lindsey_boundary_gdf) == 0:
        print("[ERROR] Could not obtain West Lindsey boundary")
        return
    ...
    bounds_27700 = west_lindsey_boundary_gdf.total_bounds  # (minx, miny, maxx, maxy)
    min_x, min_y, max_x, max_y = bounds_27700
    
    print(f"\nWest Lindsey boundary info (EPSG:27700):")
    print(f"  Geometry type: {west_lindsey_boundary.geom_type}")
    print(f"  Bounds: X[{min_x:.1f}, {max_x:.1f}], Y[{min_y:.1f}, {max_y:.1f}]")
    
    # Create bbox for quick filtering
    west_lindsey_bbox = box(min_x, min_y, max_x, max_y)

    # ----- 2. Load flood data as GeoDataFrame -----
    ...
    # ----- 4. Clip to West Lindsey TRUE administrative boundary -----
    print("Clipping to West Lindsey administrative boundary...")
    ...
    if len(west_lindsey_gdf) == 0:
        print("[ERROR] No flood events found in West Lindsey area")
        return

    print(f"[INFO] Clipped to {len(west_lindsey_gdf):,} outlines in West Lindsey area")
    ...
    bounds_wgs84 = west_lindsey_boundary_wgs84.total_bounds  # (minx, miny, maxx, maxy)
    ...
    # Define West Lindsey bounds in WGS84 for map limits (from actual boundary)
    west_lindsey_bounds_wgs84 = [
        [min_lat, min_lon],  # Southwest corner
        [max_lat, max_lon],  # Northeast corner
    ]
    
    # Convert GeoDataFrame back to GeoJSON format for Folium
    west_lindsey_features = json.loads(west_lindsey_wgs84.to_json())
    west_lindsey_features = west_lindsey_features.get("features", [])
    
    # ----- 5.5. Load and check road flood coverage (if requested) -----
    road_features = []
    if include_roads and road_geojson_dir:
        road_features = load_roads_with_simple_flood_check(
            road_geojson_dir=road_geojson_dir,
            west_lindsey_gdf=west_lindsey_gdf,
            west_lindsey_bbox=west_lindsey_bbox,
            west_lindsey_boundary=west_lindsey_boundary,
            start_year=start_year,
            end_year=end_year
        )
    ...
    # ----- 7. Create Folium map with bounds restriction -----
    print("Creating Folium map...")
    # Set maximum bounds to West Lindsey area (prevents panning outside region)
    m = folium.Map(
        location=[center_lat, center_lon],
        zoom_start=zoom_start,
        tiles="CartoDB positron",
        max_bounds=west_lindsey_bounds_wgs84,  # Restrict map panning to West Lindsey area
    )

    folium.TileLayer("OpenStreetMap", name="OpenStreetMap").add_to(m)
    folium.TileLayer("CartoDB dark_matter", name="Dark Map").add_to(m)
    
    # ----- 7.5. Add West Lindsey administrative boundary outline -----
    print("Adding West Lindsey administrative boundary to map...")
    boundary_geojson = json.loads(west_lindsey_boundary_wgs84.to_json())
    ...
    boundary_layer = folium.FeatureGroup(name='West Lindsey Boundary', show=True)
    folium.GeoJson(
        boundary_geojson,
        style_function=boundary_style,
        tooltip=folium.Tooltip('West Lindsey Administrative Boundary')
    ...
    folium.GeoJson(
        {
            ...
            "features": west_lindsey_features,
        },
        name=f"West Lindsey Flood Outlines ({start_year}-{end_year})",
        style_function=style_function,
        tooltip=tooltip,
    ).add_to(m)

    # ----- 8. Add road flood coverage layer (if available) -----
    if road_features:
    ...
    if road_features:
        ...
        years_with_floods = len(west_lindsey_gdf['start_year'].unique())
        ...
        <div style="position: fixed; 
            ...
            <div style="font-weight: bold; font-size: 16px; margin-bottom: 8px;">
                West Lindsey Flood & Road Coverage ({specific_year if specific_year else f"{start_year}-{end_year}"})
            </div>
            <div style="font-size: 11px; color: #555; margin-bottom: 8px;">
                Flood Outlines: {len(west_lindsey_features):,} | {'Year: ' + str(specific_year) if specific_year else f'Analysis Period: {end_year-start_year+1} years'}
            </div>
            <div style="font-size: 11px; color: #555; margin-bottom: 8px;">
        ...
        <div style="position: fixed; 
                    ...
                    box-shadow: 2px 2px 6px rgba(0,0,0,0.3);">
            <div style="font-weight: bold; font-size: 16px; margin-bottom: 8px;">
                West Lindsey Flood Outlines ({specific_year if specific_year else f"{start_year}-{end_year}"})
            </div>
    ...
    print(f"  Flood outlines: {len(west_lindsey_features):,}")
    if road_features:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create interactive map of flood outlines for West Lindsey area (2000-2025) with optional road risk assessment"
    )
    parser.add_argument(
        "--flood-file",
    ...
    parser.add_argument(
        "--output",
        help="Output HTML filename (default west_lindsey_flood_map_<start>_<end>.html)",
    )
    parser.add_argument(
        "--road-dir",
        default="data/geojson",
        help="Directory containing road GeoJSON files (default: data/geojson)",
    )
    parser.add_argument(
        "--include-roads",
        action="store_true",
    ...
    if args.interactive:
        create_interactive_yearly_map(
            ...
            output_file=args.output if args.output else "West_Lindsey_Interactive.html"
        )
    else:
        create_west_lindsey_flood_map(
            flood_file=args.flood_file,
            start_year=args.start_year,
            end_year=args.end_year,
